...
Key: license has client_registration_code, is_registered, last_pull_at, instance_url/name.
...
1. register_handler.go — POST /api/v1/register
   - Validate client_registration_code against client_licenses table
   - Match by client_registration_code + product_slug
...
NO JWT auth on these endpoints. client_registration_code is the only auth for register.
```
...
- DirectCreate (PO): generate license_key + client_registration_code, set status
...
Registration Status: is_registered, instance_url, last_pull_at, client_registration_code (copy button).
LicenseCreatePage (PO): wizard → generates client_registration_code.
...
- `client_registration_code` is generated by Vernon App when PO creates a license, then given to client team
