...
func (r *LicenseRepo) FindByOTP(ctx context.Context, otp, productSlug string) (*domain.ClientLicense, error) {
	var row licenseRow
	q := `SELECT
	...
	if err := r.db.GetContext(ctx, &row, q, otp, productSlug); err != nil {
		if err == sql.ErrNoRows {
			return nil, domain.ErrLicenseNotFound
		}
		return nil, fmt.Errorf("LicenseRepo.FindByOTP: %w", err)
	}
	return row.toDomain(), nil
}
...
func (r *LicenseRepo) FindByCompanyAndProduct(ctx context.Context, companyID, productID uuid.UUID) (*domain.ClientLicense, error) {
	const q = `
		...
		LIMIT 1`
	var row licenseRow
	if err := r.db.GetContext(ctx, &row, q, companyID, productID); err != nil {
		if err == sql.ErrNoRows {
			return nil, domain.ErrLicenseNotFound
		}
		return nil, fmt.Errorf("LicenseRepo.FindByCompanyAndProduct: %w", err)
	}
	return row.toDomain(), nil
}
